...
# $3 is CPU
if [ -n "${3:-}" ]; then
        FLAGS="${FLAGS:+$FLAGS }--cpu=$3"
fi

# armhf is Rasbian
if [ $ARCH = armhf ]; then
        FLAGS="${FLAGS:+$FLAGS }--enable-mmal --enable-omx-rpi"
fi
...
[ $ARCH != armhf ] || sudo chroot $BUILD_DIR /bin/sh -c 'git clone --depth 1 https://github.com/raspberrypi/firmware.git firmware && mv firmware/* / && echo /opt/vc/lib > /etc/ld.so.conf.d/00-vmcs.conf && ldconfig'
